...
void my_MPI_Alltoall(void *sendbuf, int sendcount, MPI_Datatype sendtype,
    ...
    for (int i = 0; i < size; i++) {
        MPI_Isend((char *)sendbuf + i * sendcount * send_type_size,
                  sendcount, sendtype, i, 0, comm, &requests_send[i]);
    }

    // Start non-blocking receives
    for (int i = 0; i < size; i++) {
        MPI_Irecv((char *)recvbuf + i * recvcount * recv_type_size,
                  recvcount, recvtype, i, 0, comm, &requests_recv[i]);
    }
...
int main(int argc, char *argv[]) {
    ...
    my_MPI_Alltoall(sendbuf, 3, MPI_INT, recvbuf, 3, MPI_INT, MPI_COMM_WORLD);
